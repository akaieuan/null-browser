...
The sync runs both ways: edit a note's file in Obsidian or any editor, and the next time you open that note in Null, the file's title and body are adopted into the index. The parser is tolerant (extra front-matter keys are ignored, CRLF files are normalized, a `---` rule inside the body never truncates it, a file stripped of front matter is taken whole as the body), and it refuses the dangerous cases: a file that differs only in formatting changes nothing, a file older than Null's own last write is treated as a stale mirror rather than an edit, and an empty file never erases a note (editors save by truncate-then-write, and reading mid-save must not eat the copy). One honest limit: Null's own saves rewrite the front matter to just `title` and `source`, so extra keys other tools added do not survive an in-app edit — keep those in the body.
